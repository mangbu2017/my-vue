...
export default class CreateModal extends Vue {
        ...
        zookeeperNum: [
            {
                required: true,
                type: 'number',
                message: 'ZooKeeper节点数支持1-3个节点数量',
                trigger: 'blur',
            },
        ],
        des: [
    ...
    submitData() {
        ...
        request({
            method: 'POST',
            url: '/kafkaBack/instance/createInstance',
            headers: {
            ...
            data: qs.stringify({
                ...
                disk: this.form.disk,
                zookeeperNum: this.form.zookeeperNum,
                des: this.form.des,
            }),
        }).then(() => {
    ...
    getCpuSizeList() {
        request({ url: '/kafkaBack/instance/getCpuSizeList' })
            .then((data) => {
    ...
    getMemorySizeList() {
        request({ url: '/kafkaBack/instance/getMemorySizeList' })
            .then((data) => {
