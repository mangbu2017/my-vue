...
export default class Index extends Vue {
    ...
    handleCreateSuccess() {
        ...
        if (/\/spray-rabbit(?:\/1)?\/?$/.test(this.$route.path)) {
            this.keyword = '';
            this.page.current = 1;
            this.fetchInstanceListByPage();
        } else {
            this.$router.push('/spray-rabbit/1');
        }
    }
    ...
    handlePageChange(page) {
        ...
        this.$router.push(`/spray-rabbit/${page}/${this.keyword}`);
    }
    ...
    handleSearch() {
        this.$router.push(`/spray-rabbit/1/${this.keyword}`);
    }
    ...
    fetchInstanceListByPage(preventLoadingTip = false) {
        ...
        request({
            url: '/rabbitmqBack/instance/instanceList',
            params: payload,
        }).then((data) => {
            this.loading = false;
    ...
    updateInstance(payload) {
        ...
        request({
            method: 'POST',
            url: '/rabbitmqBack/instance/updateInstance',
            headers: {
    ...
    deleteInstanceList(payload) {
        request({
            method: 'POST',
            url: '/rabbitmqBack/instance/deleteInstance',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            data: qs.stringify({
