<template>
    <div class="content-wrap">
        ...
        <table class="tai-table tai-table-border tai-table-hover">
            ...
            <tbody v-else-if="list.length">
                <tr v-for="item in list" :key="`${item.instanceID}`">
                    ...
                    <td><router-link :to="`/spray-rabbit/detail/${item.instanceID}`">{{ item.instanceID }}</router-link></td>
                    <td>{{ item.name }}</td>
                    <td>{{ item.version }}</td>
                    <td>{{ item.status }}</td>
                    <td>{{ item.flavor }}</td>
                    ...
                    <td>
                        <i-button type="text" class="btn-operation" @click="handleAddNodeNum(item)">新增节点</i-button>
                    </td>
                </tr>
            </tbody>
            <tbody v-else>
        ...
        <update-modal
            ...
            @on-ok="handleUpdateNodeNumModalOk"
            @on-cancel="handleUpdateNodeNumModalCancel"
        />
    </div>
</template>
