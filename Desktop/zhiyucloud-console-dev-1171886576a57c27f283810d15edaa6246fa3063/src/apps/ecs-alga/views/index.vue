        ...
        <table class="tai-table tai-table-border tai-table-hover">
            <thead class="tai-table-header">
                <tr>
                    ...
                    <th>实例ID</th>
                    <th>名称</th>
                    <th class="tai-table-cell-narrow">运行状态</th>
                    <th>实例用量及配额</th>
                    <th>数据库类型</th>
                    <th>系列</th>
            ...
            <tbody v-else-if="list.length">
                <tr v-for="item in list" :key="`${item.instanceID}-${item.updateDate}`">
                    ...
                    <td>
                        <router-link v-if="item.statusID === 2" :to="`/ecs-alga/detail/${item.instanceID}`">{{ item.instanceID }}</router-link>
                    ...
                    <td>{{ item.status }}</td>
                    <td class="tai-table-cell-center">
                        <i-progress :percent="item.percentage" hide-info />
                        <p>{{ item.usedSize }}/{{ item.disk }}({{ item.percentage }}%)</p>
                    </td>
                    <td>{{ item.version }}</td>
                    <td>{{ item.type }}</td>
                    <td>{{ item.creator }}</td>
