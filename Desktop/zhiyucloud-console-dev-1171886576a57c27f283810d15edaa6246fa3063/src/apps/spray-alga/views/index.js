...
export default class Index extends Vue {
    ...
    handleCreateSuccess() {
        ...
        if (/\/spray-alga(?:\/1)?\/?$/.test(this.$route.path)) {
            this.keyword = '';
            this.page.current = 1;
            this.fetchInstanceListByPage();
        } else {
    ...
    handlePageChange(page) {
        ...
        this.$router.push(`/spray-alga/${page}/${this.keyword}`);
    }
    ...
    handleSearch() {
        this.$router.push(`/spray-alga/1/${this.keyword}`);
    }
    ...
    fetchInstanceListByPage(preventLoadingTip = false) {
        ...
        request({
            url: '/mysqlclientBack/instance/instanceList',
            params: payload,
        }).then((data) => {
            this.loading = false;
    ...
    updateInstance(payload) {
        ...
        request({
            method: 'POST',
            url: '/mysqlclientBack/instance/updateInstance',
            headers: {
    ...
    deleteInstanceList(payload) {
        request({
            method: 'POST',
            url: '/mysqlclientBack/instance/deleteInstance',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            data: qs.stringify({
