<template>
    <div class="content-wrap">
        <div class="tab-wrap">
            ...
            <i-button
                v-try-back="{path: '/ecs-trepang/:page(\\d+)?/:keyword?', fallback: '/ecs-trepang'}"
                class="return-list"
                type="primary"
            >返回实例列表</i-button>
        </div>
        ...
        <div v-if="instance.mongodbInstance" class="message-item">
            ...
            <table class="tai-table tai-table-border">
                <tbody>
                    ...
                    <tr>
                        <td>状态</td>
                        <td>{{ instance.mongodbInstance.status }}</td>
                        <td>数据库版本</td>
                        <td>{{ instance.mongodbInstance.version }}</td>
                    </tr>
                    <tr>
                        <td>节点数</td>
                        <td>{{ instance.mongodbInstance.type }}</td>
                        <td>规格</td>
                    ...
                    <tr>
                        ...
                        <td>公网IP</td>
                        <td>{{ instance.mongodbInstance.isBoundPublic }}</td>
                    </tr>
                    <tr>
                        <td>创建人</td>
                        <td>{{ instance.mongodbInstance.creator }}</td>
                        <td>创建时间</td>
            ...
            <table class="tai-table tai-table-border">
                <tbody>
                    <tr>
                        ...
                        <td>节点1(公网地址)</td>
                        <td>{{ instance.node1.publicHost }}</td>
                    </tr>
                    <tr v-if="instance.node2">
                        <td>节点2(内网地址)</td>
                        <td>{{ instance.node2.innerHost }}</td>
                        <td>节点2(公网地址)</td>
                        <td>{{ instance.node2.publicHost }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
                ...
                <tbody>
                    <tr>
                        <td style="color: #bbb;">客户端使用Connection StringURL连接实例（***部分替换为root密码）
                            <span style="color: red;">请使用MongoDB 4.0以上版本的driver</span>
                        </td>
                    </tr>
                    <tr>
