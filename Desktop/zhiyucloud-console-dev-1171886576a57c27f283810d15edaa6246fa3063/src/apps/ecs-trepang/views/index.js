...
export default class Index extends Vue {
    ...
    handleCreateSuccess() {
        ...
        if (/\/ecs-trepang(?:\/1)?\/?$/.test(this.$route.path)) {
            this.keyword = '';
            this.page.current = 1;
            this.fetchInstanceListByPage();
        } else {
            this.$router.push('/ecs-trepang/1');
        }
    }
    ...
    handlePageChange(page) {
        ...
        this.$router.push(`/ecs-trepang/${page}/${this.keyword}`);
    }
    ...
    handleSearch() {
        this.$router.push(`/ecs-trepang/1/${this.keyword}`);
    }
    ...
    fetchInstanceListByPage(preventLoadingTip = false) {
        ...
        request({
            url: '/ecstrepangBack/instance/instanceList',
            params: payload,
        }).then((data) => {
            this.loading = false;
    ...
    deleteInstanceList(payload) {
        request({
            method: 'POST',
            url: '/ecstrepangBack/instance/deleteInstance',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            data: qs.stringify({
