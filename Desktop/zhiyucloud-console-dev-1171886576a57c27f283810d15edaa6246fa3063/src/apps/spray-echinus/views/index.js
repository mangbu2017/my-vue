...
export default class Index extends Vue {
    ...
    handleCreateSuccess() {
        ...
        if (/\/spray-echinus(?:\/1)?\/?$/.test(this.$route.path)) {
            this.keyword = '';
            this.page.current = 1;
            this.fetchInstanceListByPage();
        } else {
            this.$router.push('/spray-echinus/1');
        }
    }
    ...
    handlePageChange(page) {
        ...
        this.$router.push(`/spray-echinus/${page}/${this.keyword}`);
    }
    ...
    handleSearch() {
        this.$router.push(`/spray-echinus/1/${this.keyword}`);
    }
    ...
    fetchInstanceListByPage(preventLoadingTip = false) {
        ...
        request({
            url: '/echinusBack/instance/instanceList',
            params: payload,
        }).then((data) => {
            this.loading = false;
    ...
    updateInstance(payload) {
        ...
        request({
            method: 'POST',
            url: '/echinusBack/instance/updateInstance',
            headers: {
    ...
    deleteInstanceList(payload) {
        request({
            method: 'POST',
            url: '/echinusBack/instance/deleteInstance',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            data: qs.stringify({
