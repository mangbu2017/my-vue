...
export default class Index extends Vue {
    ...
    handleCreateSuccess() {
        ...
        if (/\/spray-seahare(?:\/1)?\/?$/.test(this.$route.path)) {
            this.keyword = '';
            this.page.current = 1;
            this.fetchInstanceListByPage();
        } else {
            this.$router.push('/spray-seahare/1');
        }
    }
    ...
    handlePageChange(page) {
        ...
        this.$router.push(`/spray-seahare/${page}/${this.keyword}`);
    }
    ...
    handleSearch() {
        this.$router.push(`/spray-seahare/1/${this.keyword}`);
    }
    ...
    fetchInstanceListByPage(preventLoadingTip = false) {
        ...
        request({
            url: '/redisserverBack/instance/instanceList',
            params: payload,
        }).then((data) => {
            this.loading = false;
    ...
    deleteInstanceList(payload) {
        request({
            method: 'POST',
            url: '/redisserverBack/instance/deleteInstance',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            data: qs.stringify({
