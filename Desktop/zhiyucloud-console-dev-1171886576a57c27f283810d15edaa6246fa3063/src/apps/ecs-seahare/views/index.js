...
export default class Index extends Vue {
    ...
    handleCreateSuccess() {
        ...
        if (/\/ecs-seahare(?:\/1)?\/?$/.test(this.$route.path)) {
            this.keyword = '';
            this.page.current = 1;
            this.fetchInstanceListByPage();
        } else {
            this.$router.push('/ecs-seahare/1');
        }
    }
    ...
    handlePageChange(page) {
        ...
        this.$router.push(`/ecs-seahare/${page}/${this.keyword}`);
    }
    ...
    handleSearch() {
        this.$router.push(`/ecs-seahare/1/${this.keyword}`);
    }
    ...
    fetchInstanceListByPage(preventLoadingTip = false) {
        ...
        request({
            url: '/ecsSeahareBack/instance/instanceList',
            params: payload,
        }).then((data) => {
            this.loading = false;
    ...
    updateInstance(payload) {
        ...
        request({
            method: 'POST',
            url: '/ecsSeahareBack/instance/updateInstance',
            headers: {
    ...
    deleteInstanceList(payload) {
        request({
            method: 'POST',
            url: '/ecsSeahareBack/instance/deleteInstance',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            data: qs.stringify({
