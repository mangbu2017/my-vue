...
export default class Index extends Vue {
    ...
    handleCreateSuccess() {
        ...
        if (/\/ecs-alga(?:\/1)?\/?$/.test(this.$route.path)) {
            this.keyword = '';
            this.page.current = 1;
            this.fetchInstanceListByPage();
        } else {
    ...
    handlePageChange(page) {
        ...
        this.$router.push(`/ecs-alga/${page}/${this.keyword}`);
    }
    ...
    handleSearch() {
        this.$router.push(`/ecs-alga/1/${this.keyword}`);
    }
    ...
    fetchInstanceListByPage(preventLoadingTip = false) {
        ...
        request({
            url: '/ecsalgaBack/instance/instanceList',
            params: payload,
        }).then((data) => {
            this.loading = false;
    ...
    updateInstance(payload) {
        ...
        request({
            method: 'POST',
            url: '/ecsalgaBack/instance/updateInstance',
            headers: {
    ...
    deleteInstanceList(payload) {
        request({
            method: 'POST',
            url: '/ecsalgaBack/instance/deleteInstance',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            data: qs.stringify({
