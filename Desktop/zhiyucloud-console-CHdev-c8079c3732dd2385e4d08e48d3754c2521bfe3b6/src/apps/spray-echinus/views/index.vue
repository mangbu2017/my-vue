        ...
        <table class="tai-table tai-table-border tai-table-hover">
            <thead class="tai-table-header">
                <tr>
                    ...
                    <th>实例用量及配额</th>
                    <th class="tai-table-cell-narrow">版本</th>
            ...
            <tbody v-else-if="list.length">
                <tr v-for="item in list" :key="`${item.instanceID}-${item.updateDate}`">
                    ...
                    <td><router-link :to="`/spray-echinus/detail/${item.instanceID}`">{{ item.instanceID }}</router-link></td>
                    <td>
                        {{ item.name }}
                        <i-icon
                            class="icon-eye"
                    ...
                    <td>{{ item.status }}</td>
                    <td class="tai-table-cell-center">
                        <i-progress :percent="item.usage" hide-info />
                        <p>{{ item.used }}/{{ item.disk }}({{ item.usage }}%)</p>
                    </td>
                    <td>{{ item.version }}</td>
                    <td>{{ item.flavor }}</td>
                    <td>{{ item.creator }}</td>
