...
$(function () {
    ...
    function sendAjax(reValue) {
        ...
        seriesName1 = '';
        seriesName2 = '';
        $.get('/mysqlclientBack/monitor/searchInstanceInfo', reValue, function (data) {
            var code = data.code;
            if (code === 0) {
                var res = data.resultBean;
                if (res != null && res != undefined) {
                    //初始加载
                    storageYAxis = res.verticalDesc;
                    seriesName1 = res.seriesName1;
                    seriesName2 = res.seriesName2;
                    var storagedata = res.tableIssues;
    ...
    $('#week').click(function () {
        ...
        var params = {
            instanceID: id,
            startTime: Date.parse(getDay(-5)),
            endTime: Date.parse(getDay(1)),
            timeUnit: 2,
            interval: 1,
            infoType: infoType
        };
    ...
    $('#mouth').click(function () {
        ...
        var params = {
            instanceID: id,
            startTime: Date.parse(getDay(-29)),
            endTime: Date.parse(getDay(1)),
            timeUnit: 2,
            interval: 1,
            infoType: infoType
